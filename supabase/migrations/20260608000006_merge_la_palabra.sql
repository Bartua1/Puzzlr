-- 1. Merge group_games: update la_palabra to wordle_es, handling unique constraint conflicts
INSERT INTO public.group_games (group_id, game_id, added_at)
SELECT group_id, 'wordle_es', added_at
FROM public.group_games
WHERE game_id = 'la_palabra'
ON CONFLICT (group_id, game_id) DO NOTHING;

DELETE FROM public.group_games WHERE game_id = 'la_palabra';

-- 2. Merge daily_scores: resolve duplicate constraints before updating la_palabra to wordle_es
DELETE FROM public.daily_scores ds1
USING public.daily_scores ds2
WHERE ds1.profile_id = ds2.profile_id
  AND ds1.solved_date = ds2.solved_date
  AND ds1.game_id = 'la_palabra'
  AND ds2.game_id = 'wordle_es';

UPDATE public.daily_scores
SET game_id = 'wordle_es'
WHERE game_id = 'la_palabra';

-- 3. Update wordle_es display name and delete la_palabra duplicate game
UPDATE public.games
SET display_name = 'La Palabra del Día'
WHERE id = 'wordle_es';

DELETE FROM public.games WHERE id = 'la_palabra';

-- 4. Redefine submit_daily_score_rpc to unify Spanish Wordle (wordle_es) completion message
